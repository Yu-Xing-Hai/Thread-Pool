...
1. **ABA问题**（最经典的缺陷）：
   ...
   - **解决方法**：给变量增加版本号（如`std::atomic<std::pair<T, int>>`），CAS时同时检查值和版本号
     - ABA 问题的本质是：CAS 只关注 “值的当前状态”，而忽略了 “值的历史变化”。当值关联着其他逻辑状态（如资源、指针、交易记录）时，中间的修改会导致 “值相同但逻辑不同”，CAS 的误判会引发数据不一致。而版本号的核心作用，就是用 “唯一标识” 记录值的历史变化，让 CAS 能感知到 “中间是否被修改过”，从而避免误判
2. **循环重试开销**：
